...
void Gump::GumpRectToScreenSpace(Common::Rect &rect, RectRoundDir r) {
	...
	int32 x1 = rect.left, y1 = rect.top;
	int32 x2 = rect.right, y2 = rect.bottom;
	GumpToScreenSpace(x1, y1, tl);
	GumpToScreenSpace(x2, y2, br);
	rect.left = x1;
	rect.top = y1;
	rect.right = x2 > x1 ? x2 : x1;
	rect.bottom = y2 > y1 ? y2 : y1;
}

// Transform a rectangle to gumpspace from screenspace
void Gump::ScreenSpaceToGumpRect(Common::Rect &rect, RectRoundDir r) {
	PointRoundDir tl = (r == ROUND_INSIDE ? ROUND_BOTTOMRIGHT : ROUND_TOPLEFT);
	PointRoundDir br = (r == ROUND_OUTSIDE ? ROUND_BOTTOMRIGHT : ROUND_TOPLEFT);
...
bool Gump::loadData(Common::ReadStream *rs, uint32 version) {
	...
	_dims = Common::Rect(dx, dy, dx + dw, dy + dh);
